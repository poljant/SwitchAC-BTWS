...
#include "../SwitchAC-BTWS/WebPagers.h"
#ifdef THERMOMETER
...
#ifdef IP_STATIC
IPAddress IPadr(10,110,3,87); //stały IP
...
unsigned long timestartreadtemperature;
unsigned long timereadtemperature;
unsigned long timedelayreadtemperature=750;  //czas czekania aż temperatury zostaną ustalone
unsigned long timenexreadtemperature=60000;  //czekaj na ponowny odczyt temperatury (1 min)
#endif
//dane dla AP
...
#ifdef THERMOMETER
	if(!readtemperature and millis()>timestartreadtemperature){
		sensors.requestTemperatures(); //uruchom odczyt czyjników temperatury
		timereadtemperature = millis()+timedelayreadtemperature;
		readtemperature = true;
	}
	if(readtemperature and (millis()>timereadtemperature)){
		temp = sensors.getTempCByIndex(0); //czytaj temperaturę w ºC
		timestartreadtemperature= millis()+timenexreadtemperature;
